...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/garbagebin/checkinfo/">司机列表</a></li>
		<li class="active"><a href="${ctx}/garbagebin/checkinfo/form?id=${ocrDriver.id}">司机<shiro:hasPermission name="drive:ocrDriver:edit">${not empty ocrDriver.id?'编辑':'添加'}</shiro:hasPermission><shiro:lacksPermission name="drive:ocrDriver:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="ocrDriver" action="${ctx}/drive/ocrDriver/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
			<c:if test="${empty ocrDriver.id}">
			<form:input path="loginName" placeholder="请输入账号" htmlEscape="false" maxlength="100" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</c:if>
				<c:if test="${not empty ocrDriver.id}">
					<form:hidden path="loginName"/>
					<label>${ocrDriver.loginName}</label>
				</c:if>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
			<!-- 
				<form:input path="password" placeholder="请输入密码" htmlEscape="false" maxlength="32" type="password" class="input-xlarge ${empty ocrDriver.id?'required':''}"/>
