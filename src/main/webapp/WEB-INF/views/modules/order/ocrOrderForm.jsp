...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/order/ocrOrder/">订单列表</a></li>
		<li class="active"><a href="${ctx}/order/ocrOrder/form?id=${ocrOrder.id}">订单<shiro:hasPermission name="order:ocrOrder:edit">${not empty ocrOrder.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="order:ocrOrder:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="ocrOrder" action="${ctx}/order/ocrOrder/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="departureDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${ocrOrder.departureDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			...
			<div class="controls">
				<input name="planArrivalDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${ocrOrder.planArrivalDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="order:ocrOrder:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
