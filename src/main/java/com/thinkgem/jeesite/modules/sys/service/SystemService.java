...
public class SystemService extends BaseService implements InitializingBean {
	...
	public User getForArea(User user) {
		return userDao.getForArea(user);
	}
	/**
	 * 根据登录名获取用户
	 * @param loginName
	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		// 设置分页参数
		user.setPage(page);
		// 添加状态值 by gaoly begin 2017-08-03
		List<User> userList = userDao.findList2(user);
		if (userList != null) {
			for (User userObject : userList) {
				if ("1".equals(userObject.getLoginFlag())) {
					userObject.setLoginFlag("启用");
				} else if ("0".equals(userObject.getLoginFlag())) {
					userObject.setLoginFlag("禁用");
				}
			}
		}
		// 添加状态值 by gaoly end 2017-08-03
		// 执行分页查询
	...
	public void saveUser(User user) {
		...
		if (StringUtils.isNotBlank(user.getId())){
			...
			if (user.getAreaList() != null && user.getAreaList().size() > 0){
				...
				throw new ServiceException(user.getLoginName() + "没有设置地区！");
			}
			// 将当前用户同步到Activiti
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public int deleteUserArea(User user){
		return userDao.deleteUserArea(user);
	}
